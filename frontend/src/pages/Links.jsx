        ...
        {link && link.length > 0 && link.map((data, index) => (
          <div key={index} className='bg-[#F4F6FA] flex justify-between p-4'>
            <div className='flex gap-5' >
              ...
              <div >
                <h3 className=' font-medium text-xl text-[#2C4867]'>{data.URLname}</h3>
                <p className=' mt-3 font-semibold text-sm text-[#3E6B9B]'>http://localhost:8000/{data.shortURL}</p>
                {/* <p className='  text-sm font-medium text-[#8997A6]'>{data.originalURL}</p> */}
                <p className='text-sm font-medium text-[#8997A6]'>
                  {data.originalURL.length > 15 ? data.originalURL.substring(0, 55) + '....' : data.originalURL}
                </p>

                <div className=' inline-flex gap-5 mt-5'>
                  <p onClick={() => navigate('/Analytics', { state: { analytics: data } })} className=' cursor-pointer inline-flex gap-1 items-center font-medium text-xs text-[#2C2C2C]'>
